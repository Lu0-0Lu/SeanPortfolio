...
// Skills remain hardcoded since they are usually simple layout badges
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // 4. Fetch Posts (Sorts books and poetry into separate states)
    fetch('http://localhost:5000/api/posts')
      .then((res) => res.json())
      .then((data) => {
        setBookReviews(data.filter((post) => post.post_type === 'book_review'));
        setPoetry(data.filter((post) => post.post_type === 'poetry'));
      })
      .catch((err) => console.error(err));
  }, []);
        ...
        <MainLayout>
          <main className="space-y-16 pb-16 pt-10 sm:pt-16">
                ...
                <div className="mt-6 space-y-4">
                  ...
                  {featuredProject ? (
                    <>
                      ...
                      {/* Optional: Tiny preview thumbnail if images exist */}
            ...
            <div className="grid gap-8">
                ...
                projects.map((project, index) => (
                    ...
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                        ...
                        {/* Only the first project gets the "Active Concept" badge */}
                        {index === 0 && (
                   ...
                   {/* RENDER MEDIA: Gallery Collage & Video */}
            ...
            <section id="books" className="space-y-6">
              <p className="text-sm font-medium uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Reading & Reviews</p>
              <div className="grid gap-6 sm:grid-cols-2">
                {bookReviews.map((book) => (
                  <article key={book.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-slate-400 hover:shadow-xl dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-500">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h4 className="text-lg font-semibold text-slate-900 dark:text-white">{book.title}</h4>
                        <p className="text-sm text-slate-600 dark:text-slate-400">{book.author}</p>
                      </div>
                      <span className="rounded-full border border-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-700 dark:border-slate-700 dark:text-slate-300">
                        {book.rating}
                      </span>
                    </div>
                    <p className="mt-4 text-sm leading-6 text-slate-600 dark:text-slate-300">{book.content}</p>
                  </article>
                ))}
              </div>
            </section>
            ...
            <section id="hobbies" className="space-y-6">
              <p className="text-sm font-medium uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Beyond the Code</p>
              <div className="grid gap-6 sm:grid-cols-2">
                {poetry.map((poem) => (
                  <div key={poem.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-slate-400 hover:shadow-xl dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-500">
                    <h4 className="text-lg font-semibold text-slate-900 dark:text-white">{poem.title}</h4>
                    <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{poem.content}</p>
                  </div>
                ))}
              </div>
            </section>
